...
export type StoreSettings = {
  ...
  email: string;
  hours: string; // 운영시간 안내 — 화면에 그대로 보여 주는 글자
  /**
   * ============================================================
   * ** 상담 가능 시간 — 숫자로 (2026-08-27)
   * ============================================================
   *
   * ** 위 hours 는 사람이 읽는 글자입니다. 컴퓨터가 "지금 상담 되나" 를
   *   판단할 수 없습니다. 그래서 숫자 칸을 따로 둡니다.
   *   hours 글자는 그대로 둡니다. 화면 표시용으로 계속 씁니다.
   *
   * ** 요일별로 일곱 줄을 만들지 않습니다. 평일 한 묶음 + 토요일이면
   *   충분합니다. 일요일은 칸 없이 자동 휴무입니다.
   *
   * ** 시각은 소수로 적습니다. 9 = 09:00, 12.5 = 12:30, 17 = 17:00
   *   시·분을 따로 두면 칸이 두 배가 되고 계산이 번거로워집니다.
   *
   * ** 점심은 평일·토요일 공통입니다. (사장님 판단, 2026-08-27)
   *   토요일 점심을 다르게 하시려면 칸 두 개만 더하면 됩니다.
   */
  /** 평일 상담 시작 (예: 9 = 09:00) */
  weekdayOpen: number;
  /** 평일 상담 끝 (예: 17 = 17:00) */
  ...
  lunchEnd: number;
  /**
   * 쉬는 날. 한 줄에 쉼표로 적습니다. 예: 2026-01-01, 2026-03-01
   *
   * ** 연 1회 붙여넣기입니다. (사장님 판단)
   *   공공데이터에서 자동으로 가져오는 방법은 키 발급과 장애 처리가
   *   붙습니다. 연 1회 5분이면 되는 일에 그럴 값어치가 없습니다.
   * * 주말은 여기 안 적어도 됩니다. 저절로 판단합니다.
   */
  holidays: string;
...
export type ShippingSettings = {
  ...
  leadTime: string;
  /**
   * 배송중인 주문을 며칠 뒤 자동으로 배송완료로 넘길지.
   *
   * ★★ 왜 필요한가
   *   배송이 끝나도 아무도 상태를 바꿔 주지 않으면 주문은 영원히 배송중입니다.
   *   구매 적립 포인트는 배송완료 시점에 나가므로 손님은 포인트를 못 받고,
   *   후기도 쓸 수 없습니다. 사람 손을 기다리게 두면 안 되는 자리입니다.
   *
   * ★ 송장이 실제로 있는 주문만, 배송중이 된 지 이 일수가 지났을 때만 넘깁니다.
   *   기준은 주문일이 아니라 배송중으로 바뀐 시각입니다.
   * ★ 0 이면 자동으로 넘기지 않습니다. 사람이 직접 바꿔야 합니다.
   */
  autoDeliveredDays: number;
  /**
...
export const DEFAULT_SHIPPING: ShippingSettings = {
  ...
  /*
   * ★ 배송·교환·반품 안내의 "배송 소요일" 한 줄입니다. 표 안에 들어가 짧아야 합니다.
   * ★ 예전에는 "1~3영업일 내 출고" 였는데 실제로는 당일 출고입니다.
   *   1~3영업일은 출고가 아니라 도착까지의 시간입니다.
   */
  leadTime:
    '오후 2시 이전 주문은 당일 출고 · 보통 1~3영업일 내 도착 (최대 7영업일)',
  productLine: '',
  /*
   * ★ 7일 — 택배가 아무리 늦어도 이 안에는 도착합니다.
   *   너무 짧으면 아직 못 받은 손님의 주문이 배송완료로 넘어가고,
   *   너무 길면 포인트가 그만큼 늦게 나갑니다.
   */
  autoDeliveredDays: 7,
};
...
export const DEFAULT_SALES: SalesSettings = {
  ...
  deliveryPeriod: [
     ...
     */
    '오후 2시 이전에 주문하시면 당일 출고되고, 그 이후 주문은 다음 영업일에 출고됩니다.',
    '받으시기까지는 보통 1~3영업일이 걸리며, 늦어도 결제 확인일로부터 최대 7영업일 이내에 배송해 드립니다.',
    '주말·공휴일은 출고되지 않으며, 연휴나 기상 상황에 따라 늦어질 수 있습니다. 지연이 예상되면 미리 안내드립니다.',
  ].join('\n'),
  exchangePolicy: [
