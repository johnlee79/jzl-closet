...
export type CreatePaymentResult =
  ...
  | {
      /**
       * PG 결제창을 폼으로 열어야 하는 경우 (4-A · KSNET).
       *
       * ★ 왜 redirect 로 안 되는가
       *   KSNET 은 주소 하나로 여는 방식이 아닙니다.
       *   파라미터 스무 개쯤을 폼에 담아 POST 해야 하고, PC 와 모바일의
       *   여는 방법도 다릅니다. (PC = 자바스크립트 레이어, 모바일 = 페이지 이동)
       *   그래서 "어디로 보낼지" 가 아니라 "무엇을 담아 어떻게 열지" 를 돌려줍니다.
       * ★ 실제 폼은 /checkout/pay 화면이 서버에서 다시 만듭니다.
       *   여기 값은 그 화면으로 보낼지 판단하는 데만 씁니다.
       */
      kind: 'pg_form';
      /** 결제를 이어서 진행할 우리 쪽 화면 */
      continueUrl: string;
