    ...
    <div className="mx-auto w-full max-w-[1100px] px-5 py-14 md:px-10 md:py-20">
      ...
      {/*
        ...
        * 연락처 안내와 동시에 두 개가 뜨지 않게 합니다. 하나씩 처리하는 편이
          실제로 채워집니다.
        ============================================================
        ** 이 판정은 /api/auth/me 와 같아야 합니다 (2026-08-26)
        ============================================================

          같은 것을 두 곳에서 판단하고 있습니다.
            여기            서버에서 그리는 마이페이지 안 배너
            /api/auth/me    헤더 아래 배너(SiteNotices)가 보는 값

          ** 한쪽만 고치면 조용히 어긋납니다.
            화면 두 곳이 서로 다른 말을 하게 되는데, 아무 데도 오류가 나지
            않아 알아채기 어렵습니다. 회원 상태 판정을 lib/member-session.ts
            한 곳으로 모았던 것과 정반대 모양입니다.

          ** 언젠가 클라이언트 컴포넌트로 합치는 것이 맞습니다.
            이 블록을 작은 클라이언트 컴포넌트로 바꿔 useMember() 를 쓰면
            판정이 /api/auth/me 한 곳만 남습니다. 지금은 잘 돌고 있어
            건드리지 않았습니다. (사장님 판단, 2026-08-26)
      */}
      {member.profile.phone &&
