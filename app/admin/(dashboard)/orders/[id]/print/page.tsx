...
export default async function OrderPrintPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[800px]">
      ...
      <article className="rounded-lg border border-slate-300 bg-white p-8 text-[13px] leading-relaxed text-slate-900 print:rounded-none print:border-0 print:p-0">
        <header className="flex flex-wrap items-start justify-between gap-4 border-b-2 border-slate-900 pb-4">
          <div>
            <p className="text-[18px] font-semibold tracking-[0.2em]">{store.name}</p>
            <p className="mt-1 text-[12px] text-slate-600">거래명세 · 주문서</p>
          </div>
          <div className="text-right">
            <p className="font-mono text-[16px] font-semibold">{order.orderNo}</p>
            <p className="mt-1 text-[12px] text-slate-600">
              {order.createdAt ? new Date(order.createdAt).toLocaleString('ko-KR') : ''}
            </p>
            <p className="mt-0.5 text-[12px]">{statusLabel(order.status)}</p>
          </div>
        </header>

        <div className="mt-6 grid grid-cols-2 gap-6">
          <section>
            <h2 className="border-b border-slate-300 pb-1 text-[13px] font-semibold">
              주문자
            </h2>
        ...
        <dl className="ml-auto mt-4 flex w-full max-w-[280px] flex-col gap-1">
          ...
          <div className="flex justify-between border-t border-slate-900 pt-1 text-[15px] font-semibold">
            <dt>합계</dt>
