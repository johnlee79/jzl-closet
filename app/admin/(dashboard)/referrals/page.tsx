...
export default async function AdminReferralsPage({
  ...
  return (
    <div className="mx-auto w-full max-w-[1100px]">
      <h1 className="text-[24px] font-semibold text-slate-900">추천 관리</h1>
        ...
        <dl className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            ...
            <div key={item.label} className="border border-slate-200 p-4">
              <dt className="text-[14px] text-slate-500">{item.label}</dt>
              <dd className="mt-1 text-[24px] font-semibold text-slate-900">
                {item.value}
              </dd>
            </div>
          ))}
      ...
      {/* ── 설정 ────────────────────────────────────── */}
      <section className="admin-card mt-5 p-5">
        <h2 className="text-[17px] font-semibold text-slate-900">추천 설정</h2>
      ...
      <section className="admin-card mt-5 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-[17px] font-semibold text-slate-900">추천인별 현황</h2>
          <form method="get" className="flex flex-wrap items-center gap-2">
        ...
        {summaries.length === 0 ? (
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-[16px]">
              <thead>
                <tr className="border-b border-slate-200 text-[15px] text-slate-500">
                ...
                {summaries.map((row) => (
                  <tr key={row.id} className="border-b border-slate-100">
                    <td className="px-3 py-2.5">
                      ...
                      <span className="ml-2 text-[14px] text-slate-500">{row.email}</span>
                    </td>
