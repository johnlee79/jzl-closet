...
export default async function AdminStatsPage({
  ...
  return (
    <div className="mx-auto w-full max-w-[1100px]">
      <h1 className="text-[20px] font-semibold text-slate-900">통계</h1>
      ...
      <div className="admin-card mt-5 flex flex-wrap items-center justify-between gap-3 border-blue-200 bg-blue-50 p-4">
        <p className="text-[14px] leading-relaxed text-blue-900">
          ...
          {analytics.ga4Id ? (
            <span className="ml-2 font-mono text-[13px]">{analytics.ga4Id}</span>
          ) : (
      ...
      <section className="mt-8">
        <h2 className="text-[16px] font-semibold text-slate-900">일자별 매출</h2>
        {sales.daily.length === 0 ? (
          <p className="admin-card mt-3 px-4 py-10 text-center text-[14px] text-slate-500">
            이 기간에는 주문이 없습니다.
          </p>
        ) : (
      ...
      {/* ── 상태별 건수 ───────────────────────────────── */}
      <section className="mt-8">
        <h2 className="text-[16px] font-semibold text-slate-900">상태별 주문 건수</h2>
        <ul className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {ORDER_STATUSES.map((status) => (
            <li key={status} className="admin-card p-3">
              ...
              <span className="mt-2 block text-[18px] font-semibold tabular-nums text-slate-900">
      ...
      {/* ── 상품 통계 ─────────────────────────────────── */}
      <section className="mt-8">
        <h2 className="text-[16px] font-semibold text-slate-900">판매 수량 상위 20</h2>
        <div className="admin-card mt-3 overflow-x-auto">
          {products.top.length === 0 ? (
            <p className="px-4 py-10 text-center text-[14px] text-slate-500">
              이 기간에는 판매된 상품이 없습니다.
            </p>
          ) : (
      ...
      <div className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-2">
        <section>
          <h2 className="text-[16px] font-semibold text-slate-900">카테고리별 매출</h2>
          <div className="admin-card mt-3 p-4">
            {products.byCategory.length === 0 ? (
              <p className="py-6 text-center text-[14px] text-slate-500">자료가 없습니다.</p>
            ) : (
        ...
        <section>
          <h2 className="text-[16px] font-semibold text-slate-900">브랜드별 매출</h2>
          <div className="admin-card mt-3 p-4">
      ...
      <div className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-2">
        <section>
          <h2 className="text-[16px] font-semibold text-slate-900">리뷰가 많은 상품</h2>
        ...
        <section>
          <h2 className="text-[16px] font-semibold text-slate-900">별점이 낮은 상품</h2>
          <div className="admin-card mt-3">
