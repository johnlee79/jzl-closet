...
function Delta({ current, previous }: { current: number; previous: number }) {
  ...
  if (rate === null) {
    return <span className="text-[14px] text-slate-400">비교 대상 없음</span>;
  }
  const up = rate > 0;
  const flat = rate === 0;
  return (
...
function StatCard({
  ...
  const body = (
    <>
      ...
      >
        {formatPrice(value)}
        {suffix ? <span className="ml-1 text-[16px] font-normal">{suffix}</span> : null}
      </p>
...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1280px]">
      <h1 className="text-[24px] font-semibold text-slate-900">대시보드</h1>
      ...
      <section className="mt-8">
        <h2 className="text-[18px] font-semibold text-slate-900">상태별 주문</h2>
            ...
            return (
              <li key={status}>
                ...
                >
                  ...
                  <span className="mt-2 block text-[24px] font-semibold tabular-nums text-slate-900">
                    {count}
                  </span>
                </Link>
              </li>
      ...
      <section className="mt-8">
        ...
        <div className="admin-card mt-3 overflow-x-auto">
          {stats.recentOrders.length === 0 ? (
            <p className="px-4 py-14 text-center text-[16px] leading-relaxed text-slate-500">
              아직 주문이 없습니다.
              <br />
