            ...
            {editing === gift.id ? (
              ...
              <div className="flex flex-wrap items-center gap-4 border border-slate-200 p-4">
                {gift.imageUrl ? (
                  ...
                  <div className="flex h-[64px] w-[86px] shrink-0 items-center justify-center bg-slate-100 text-[12px] text-slate-400">
                    이미지 없음
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  <p className="text-[15px] text-slate-900">{gift.name}</p>
                  {gift.description ? (
                    <p className="mt-1 text-[13px] text-slate-600">{gift.description}</p>
                  ) : null}
                  {!gift.isVisible ? (
