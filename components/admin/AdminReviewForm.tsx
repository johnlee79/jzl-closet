...
export default function AdminReviewForm({
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-4">
      {error ? (
        <p role="alert" className="rounded-md bg-red-50 px-3 py-2 text-[14px] text-red-700">
          {error}
        </p>
      ) : null}
            ...
            {attachments.map((url) => (
              <li key={url} className="relative">
                ...
                <button
                  ...
                  aria-label="사진 삭제"
                  className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-black/60 text-[13px] leading-none text-white"
                >
                  ×
                </button>
              </li>
      ...
      >
        <label className="flex cursor-pointer items-start gap-3 text-[14px] text-slate-900">
          <input
            type="checkbox"
            checked={isSponsored}
