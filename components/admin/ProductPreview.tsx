...
import { brandLabel as findBrandLabel, brandName as findBrandName, type Brand } from '@/lib/brands';
 ...
 *
 * 분류·브랜드는 DB 에서 오므로 서버 페이지가 읽어 props 로 넘겨 줍니다.
 */
export default function ProductPreview({
  ...
  const brandName = draft.brandSlug ? findBrandName(brands, draft.brandSlug) : storeName;
  const brandLabel = draft.brandSlug ? findBrandLabel(brands, draft.brandSlug) : '';
  const category = findCategory(categories, draft.categorySlug);
  const sub = draft.subCategorySlug
    ? findSubCategory(categories, draft.categorySlug, draft.subCategorySlug)
    : undefined;
  const discount = getDiscountRate(draft);

  return (
    <article className="shell py-10 md:py-14">
      <p className="mb-6 rounded-sm border border-wine px-4 py-3 text-[16px] text-wine">
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-16">
        ...
        <section aria-label="상품 정보" className="lg:pt-4">
          {brandLabel ? (
            <p className="text-[15px] tracking-[0.16em] text-muted">{brandLabel}</p>
          ) : null}
          <h1 className="mt-3 font-serif text-[30px] leading-snug text-ink md:text-[36px]">
            {draft.name || '상품명 미입력'}
          </h1>
          <p className="mt-4 text-[18px] leading-[1.9] text-ink md:text-[19px]">
            {draft.summary}
          </p>
          ...
          {draft.optionGroups.length > 0 ? (
            <div className="mt-8 flex flex-col gap-1.5 border-t border-stone pt-6">
              {draft.optionGroups.map((group) => (
                <p key={group.name} className="text-[16px] text-ink">
